...
<div class="container">
  ...
  <div class="control-group">
    <div class="controls">
      <input type="text" class="select2" name="baslik" placeholder="Başlık girin" autocomplete="off">
    </div>
  </div>
  <!-- buton -->
	<ul class="pButon">
	...
	<div class="pEditor" contenteditable="true">
		<?php
			/*echo $_POST['pTextarea'] ? stripslashes($_POST['pTextarea']) :*/ 'İçeriği düzenlemek için tıklayın!';
		?>
	</div>
	<!--/editör -->
  ...
  <div class="control-group">
    ...
    <select size="1" onChange="showUser(this.value)" name="users" class="select" style="margin-top:15px;">
    
    <option value="---">Ana kategori</option>
	<?php
    
		$vericek="Select * from tbl_kategoriler";
		...
		for($i=0;$i<$say;$i++){
			...
			$satir=mysql_fetch_array($sql);
				...
				echo "<option value='".$satir['KategoriId']."'>".$satir['KategoriAdi']."</option>";
  ...
  <div id="txtHint">
  	...
  	<div class="control-group">
        ...
        <select size="1" name="" class="select">
  ...
  <div class="control-group">
    <div class="controls">                  
        ...
        <?php
			...
			if(empty($baslik) || empty($altkat) || empty($_POST['pTextarea']) || empty($kullanici)){
	...
	$sqlcumlesi="insert into tbl_paylasimlar values('','$altkat','$baslik','".addslashes($icerik)."','$kullanici','$tarih','$tarih')";
